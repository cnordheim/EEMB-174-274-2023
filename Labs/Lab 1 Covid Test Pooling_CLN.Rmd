...
# Write down the three other types of bionomial function's names. 
 ...
 # rbinom, random "pulls" from the binomial distribution
```
...
*< Sign presents the "sign" of your value as either a 1, 0 , -1, if it's positive it's a 1, if its a negative its -1, if it's 0 it's 0 >* 
...
z %>%
  ...
  log()  # then take the natural log (which is the default for the log function)
...
*< In this setup, if you test positive even in a pool of 1, our code says you will retest (even though logically that doesn't make sense, we have no stipulation in the code for a pool size of 1 not requiring a retest), and thus a positive test in a pool of 1 ends up using 2 tests. Thus, we expect a mean higher than 1, and because there is a 0.05 chance of being Bd positive, a value of ~1.05 makes sense. >*
...
*Yes the pooling strategy works! The best pool size, k, is 5 people per pool, as it has the lowest mean number of tests per person.* 
...
```
*When the pool size is 5 or smaller, the average number of tests per person is lower, and thus, is more efficient. As th epool size grown, it becomes less efficient than a pool of five, but still more efficient than the one test per person, up to a pool of around 90 people. When the pool is above 90 people, it is close to a mean of one test per person, which is then comparable to the classic one test per person strategy.*